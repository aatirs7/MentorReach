...
/**
 * One mentor card, shared by browse and the homepage so the two can't drift.
 *
...
export function MentorAvatar({
  ...
  mentor: Pick<MentorCardData, 'fullName' | 'headshotUrl' | 'isSeed'>
  size?: number
  className?: string
}) {
  const source = resolveHeadshot(mentor)
...
export function MentorCard({ mentor }: { mentor: MentorCardData }) {
  return (
    <Link href={`/mentors/${mentor.userId}`} className="group block h-full">
      ...
      <Card className="flex h-full flex-col gap-0 border-line/15 bg-raised p-6 transition-all group-hover:-translate-y-0.5 group-hover:border-gold">
        <div className="flex items-center gap-3.5">
          <MentorAvatar mentor={mentor} size={52} className="text-lg" />
          <div className="min-w-0">
            <h3 className="truncate text-base leading-snug">{mentor.fullName ?? 'Mentor'}</h3>
            <p className="truncate text-xs leading-relaxed text-slate">{mentor.currentTitle}</p>
          </div>
        </div>
