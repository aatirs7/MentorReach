...
/**
 * Spec §10 — Stripe Connect Express onboarding for a mentor.
 *
 * §14.4 is open (self-serve vs. admin-assisted for the first cohort). This builds the
 * SELF-SERVE path, which an admin can also walk a mentor through — that's a superset, so
...
export default async function PayoutsPage() {
  ...
  // Provision on demand so a mentor approved before Stripe existed can still self-serve.
  let accountId = profile.stripeAccountId

  if (!accountId) {
    try {
      accountId = await createExpressAccount({ email: user.email, mentorUserId: user.id })
      await db
  ...
  try {
    ...
    // Mirror into the DB here too, not just from the account.updated webhook: a mentor who
  ...
  return (
    <Shell>
      ...
      <Button asChild variant="ghost" className="mt-6 px-0">
        <Link href="/mentor">← Back to your mentoring</Link>
      </Button>
    </Shell>
  )
}
