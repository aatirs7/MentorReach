...
/**
 * The mentor's resource hub — everything they have in one place. Doubles as the final
...
export default async function MentorResourcesPage() {
  // requireMentor so an admin previewing (view-as) also sees it; it renders no mentor data.
  await requireMentor()
  ...
  const groups: Array<{ title: string; items: Array<{ href: string; label: string; blurb: string; external?: boolean }> }> = [
    {
      ...
      items: [
        { href: '/mentor', label: 'Dashboard', blurb: 'Your publish checklist, sessions, and referral link.' },
        { href: '/mentor/setup', label: 'Edit your profile', blurb: 'Update your field, role, bio, photo, and rates.' },
    ...
    },
    {
      ...
      items: [
        { href: '/mentor/handbook', label: 'Mentor Handbook', blurb: 'How sessions run, the code of conduct, payments, and cancellations.' },
    ...
    <main className="mx-auto w-full max-w-4xl flex-1">
      <div className="text-center">
        ...
        <p className="mx-auto mt-2 max-w-prose text-slate">
          The tools, standards, and support that come with mentoring on MentorReach.
        </p>
      </div>
