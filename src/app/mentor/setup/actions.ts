 ...
 * availability, Stripe payouts, handbook — see src/lib/mentor-publish.ts). `status` is not
...
export async function saveMentorProfile(
  _prev: MentorSetupState,
  formData: FormData,
): Promise<MentorSetupState> {
  const user = await requireUser()

  if (await viewingAsMentor()) {
    return { message: 'You’re viewing as a mentor (read-only). Exit mentor view to make changes.' }
  }

  if (user.role !== 'mentor') {
    return { message: 'Only mentors can set up a mentoring profile.' }
  }
  ...
  if (existing) {
    ...
    await db.update(mentorProfiles).set(textValues).where(eq(mentorProfiles.id, existing.id))
  } else {
...
/**
 * Upload the mentor's resume/CV (PDF, Vercel Blob), separately from the text form for the
...
export async function uploadResumeAction(
  ...
  if (user.role !== 'mentor') return { message: 'Only mentors can upload a resume.' }
  ...
  try {
    ...
    console.error('[mentor-setup] resume upload failed', err)
    return { errors: { resume: ['Upload failed. Please try again.'] } }
  }

  await db.update(mentorProfiles).set({ resumeUrl: url }).where(eq(mentorProfiles.id, profile.id))

  revalidatePath('/mentor/setup')
...
/**
 * Upload the mentor's own headshot (Vercel Blob), separately from the text form so a large
 * image can't take the text save down with it, and vice versa.
 *
 * A real mentor MUST upload a photo to publish. This produces a blob URL (never a
...
export async function uploadHeadshotAction(
  ...
  if (user.role !== 'mentor') return { message: 'Only mentors can upload a headshot.' }
  ...
  try {
    ...
    console.error('[mentor-setup] headshot upload failed', err)
    return { errors: { photo: ['Upload failed. Please try again.'] } }
  }

  await db.update(mentorProfiles).set({ headshotUrl: url }).where(eq(mentorProfiles.id, profile.id))
