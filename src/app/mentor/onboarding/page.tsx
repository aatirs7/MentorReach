...
import { INDUSTRIES } from '@/lib/mentor-schema'
import { hasRealPhoto } from '@/lib/mentor-publish'
...
/**
 * The guided mentor onboarding. One step at a time; each substantive step reuses the same
 * validators and write helpers as /mentor/setup (which stays as the returning-mentor edit
 * surface). Admins can preview it read-only via "view as mentor".
 */
export default async function MentorOnboardingPage({
  ...
  // Resolve whose onboarding we're rendering: the mentor's own, or (for an admin) the mentor
  // they're previewing read-only.
  let mentorUserId = user.id
  let mentorEmail = user.email
  let viewAs = false

  if (user.role === 'admin') {
    const targetId = await readViewAsMentorId()
    if (!targetId) redirect('/admin/mentors')
    const [targetUser, targetProfile] = await Promise.all([
      db.query.users.findFirst({ where: eq(users.id, targetId) }),
      db.query.mentorProfiles.findFirst({ where: eq(mentorProfiles.userId, targetId), columns: { id: true } }),
    ])
    if (!targetUser || !targetProfile) redirect('/admin/mentors')
    mentorUserId = targetUser.id
    mentorEmail = targetUser.email
    viewAs = true
  ...
  const offerings = profile
    ? await db.query.mentorOfferings.findMany({
        where: and(eq(mentorOfferings.mentorId, mentorUserId), eq(mentorOfferings.isActive, true)),
      })
    : []
  const [availabilityRules, blackouts] = profile
    ? await Promise.all([
        ...
        db.query.mentorAvailabilityBlackouts.findMany({ where: eq(mentorAvailabilityBlackouts.mentorId, mentorUserId) }),
      ])
    : [[], []]

  // Prefill (only before a profile exists): an accepted application, else a mentor invite.
  ...
  if (!profile && !viewAs) {
    const app = await db.query.mentorApplications.findFirst({
      where: and(eq(mentorApplications.email, mentorEmail), eq(mentorApplications.status, 'accepted')),
    })
    if (app) {
      prefill = {
      ...
      const inv = await db.query.mentorInvites.findFirst({
        where: and(eq(mentorInvites.email, mentorEmail), inArray(mentorInvites.status, ['pending', 'accepted'])),
      })
      if (inv) {
        prefill = {
          industry:
    ...
    <main className="mx-auto w-full max-w-2xl flex-1 px-6 py-14">
      ...
      <div className="mt-8">
        {active === 'welcome' ? (
            ...
            <div className="space-y-10">
              ...
              <div className="text-center">
                <Button asChild size="lg">
                  <Link href="/mentor/onboarding?step=sessions">Continue</Link>
                </Button>
              </div>
            </div>
          </StepFrame>
          ...
          <StepFrame title="Your availability" intro="Set the hours you’re open to mentor. We create the Zoom meeting when someone books.">
            <AvailabilityEditor
            ...
            <div className="mt-8 text-center">
              <Button asChild size="lg">
                <Link href="/mentor/onboarding?step=payouts">Continue</Link>
              </Button>
            </div>
          </StepFrame>
        ) : active === 'payouts' ? (
          ...
          <StepFrame title="The Mentor Handbook" intro="The standards every mentor agrees to.">
            <HandbookStep
      ...
      {prev && active !== 'welcome' ? (
        <p className="mt-10 text-center">
          <Link
            href={`/mentor/onboarding?step=${prev.key}`}
    ...
    <div className="text-center">
      ...
      <Card className="mx-auto mt-8 max-w-md border-line/20 p-6 text-left">
        ...
        <ul className="mt-3 space-y-1.5 text-sm text-slate">
          ...
          <li>· The weekly hours you&rsquo;re free to mentor</li>
        </ul>
      </Card>
      <div className="mt-8">
        <Button asChild size="lg">
          <Link href="/mentor/onboarding?step=about">Let&rsquo;s go</Link>
        </Button>
      </div>
    </div>
  )
    ...
    <div className="space-y-6 text-center">
      ...
      <div className="flex flex-wrap justify-center gap-3">
        <Button asChild variant="outline">
          <Link href="/mentor/payouts">{enabled ? 'Manage payouts' : 'Set up payouts'}</Link>
        </Button>
        <Button asChild size="lg">
          <Link href="/mentor/onboarding?step=handbook">Continue</Link>
        </Button>
      </div>
    </div>
  )
...
function DoneStep({ viewAs }: { viewAs: boolean }) {
  const resources = [
    { href: '/mentor', label: 'Your dashboard', blurb: 'Your checklist, sessions, and referral link.' },
    { href: '/mentor/handbook', label: 'Mentor Handbook', blurb: 'The standards and how sessions run.' },
    { href: '/mentor/payouts', label: 'Payouts', blurb: 'Manage your Stripe payout account.' },
    { href: '/sessions', label: 'Your sessions', blurb: 'Upcoming and past bookings.' },
    { href: '/mentor/setup', label: 'Edit your profile', blurb: 'Update anything, anytime.' },
    { href: '/mentor/resources', label: 'Resources', blurb: 'Everything in one place.' },
  ]
  return (
    <div className="text-center">
      ...
      <div className="mt-10">
        {viewAs ? (
          <p className="text-sm text-slate">You&rsquo;re previewing this mentor read-only.</p>
        ) : (
          <FinishButton />
        )}
      </div>
