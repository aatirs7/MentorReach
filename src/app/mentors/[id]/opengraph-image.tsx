...
import { getPublicMentor } from '@/lib/browse'
import { formatPrice } from '@/lib/mentor-schema'

/**
 * Per-mentor share card. The profile is the link people actually paste into LinkedIn and
 * group chats, so this is the highest-traffic image on the site.
 *
 * It reuses getPublicMentor(), which returns null for anyone not live — so a suspended or
 * half-finished mentor cannot have their name and rate rendered into an image that then
 * sits in a scraper's cache long after the page itself started 404ing.
 */
export const alt = 'Mentor on MentorReach'
