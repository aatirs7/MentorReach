...
export async function slotsForOffering(offeringId: string): Promise<string[]> {
  const offering = await db.query.mentorOfferings.findFirst({
    where: eq(mentorOfferings.id, offeringId),
    columns: { mentorId: true, lengthMinutes: true, isActive: true },
  })
 ...
 *
 * THIS is where the survey gate lives (the mentor list and profile are public). It redirects
