...
export function ApplicationForm() {
  ...
  const toggle = (k: 'days' | 'times' | 'mentoringTypes', val: string) =>
    setForm((f) => ({
      ...f,
  ...
  function stepValid(s: number): boolean {
    switch (s) {
      ...
      case 5:
        return (
          form.mentoringTypes.length > 0 &&
          (!form.mentoringTypes.includes('Other') || !!form.mentoringOther.trim())
        )
      case 6:
  ...
  function submit() {
    ...
    start(async () => {
      const res = await submitApplication({
        ...
        times: form.times,
        days: form.days,
        mentoringTypes: form.mentoringTypes,
      })
      if (res.ok) {
        try {
        ...
        <div className="w-full max-w-lg">
          {step === 1 ? (
            <StepShell title="Thanks for your interest">
              <p className="leading-relaxed text-slate">
                Thanks for your interest in mentoring with MentorReach. This takes about 5 to 10
          ...
          {step === 5 ? (
            <StepShell title="What you'd offer">
              <ChipGroup
                label="What mentoring could you bring? Select all that apply."
                required
                selected={form.mentoringTypes}
                onToggle={(v) => toggle('mentoringTypes', v)}
                options={MENTORING_TYPES.map((c) => ({ value: c, label: c }))}
              />
              {form.mentoringTypes.includes('Other') ? (
                <TextField label="What else?" value={form.mentoringOther} onChange={(v) => set('mentoringOther', v)} required />
              ) : null}
              <TextAreaField
          ...
          {step === 6 ? (
            <StepShell title="Employer & visibility">
              <RadioField
                label="Would your current employer have any issues with your name/title being associated with paid mentoring on an outside platform?"
          ...
          {step === 7 ? (
            <StepShell title="Fit & motivation">
              <TextAreaField label="Why are you interested in mentoring with MentorReach?" value={form.whyInterested} onChange={(v) => set('whyInterested', v)} required />
              <TextAreaField
                label="Do you have experience with mentoring, mentoring, or tutoring? (Not an issue if you don't.)"
