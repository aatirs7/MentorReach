...
/**
 * Mentor application submit (public, no auth). Saves to mentor_applications, emails the
...
export async function submitApplication(raw: unknown): Promise<ApplyState> {
  ...
  await db.insert(mentorApplications).values({
    ...
    mentoringTypes: v.mentoringTypes as string[],
    mentoringOther: v.mentoringOther || null,
  ...
  await Promise.all(
    admins.map((admin) =>
      sendEmail({
        to: admin.email,
        subject: `New mentor application: ${v.fullName}`,
