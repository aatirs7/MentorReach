...
import { createMentorInvite, inviteUrl } from '@/lib/mentor-invite'
import { MentorInviteEmail } from '@/lib/email/templates'
...
/** Invite a pre-approved mentor directly (no application). Sends the email AND returns the
...
export async function createInviteAction(_prev: InviteState, formData: FormData): Promise<InviteState> {
   ...
   *
   * The invite itself is valid either way: createMentorInvite() has already written the
...
export async function revokeInviteAction(formData: FormData): Promise<void> {
  ...
  if (!id) return
  await db.update(mentorInvites).set({ status: 'revoked' }).where(eq(mentorInvites.id, id))
  revalidatePath('/admin/mentors')
}
...
export async function resendInviteAction(formData: FormData): Promise<void> {
  ...
  const invite = await db.query.mentorInvites.findFirst({ where: eq(mentorInvites.id, id) })
