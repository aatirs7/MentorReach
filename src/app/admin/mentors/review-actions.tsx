...
import { type AdminState, setMentorStatus } from '../actions'
import { Button } from '@/components/ui/button'

/** Suspend / reinstate a mentor — the only admin lever now that approval is gone. */
export function StatusActions({ profileId, suspended }: { profileId: string; suspended: boolean }) {
  const [state, action, pending] = useActionState<AdminState, FormData>(setMentorStatus, {})
    ...
    <div className="mt-4 border-t border-line/15 pt-4">
      {confirming ? (
        <form action={action}>
          ...
          <p className="mb-2 text-sm text-slate">Take this mentor offline?</p>
