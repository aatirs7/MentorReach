...
import { aboutStepSchema, parsePriceToCents, sessionsStepSchema } from '@/lib/coach-schema'
...
/** Shared guard: onboarding is a coach-only, write path — admins previewing can't save. */
async function requireEditingCoach(): Promise<
  { ok: true; user: Awaited<ReturnType<typeof requireUser>> } | { ok: false; state: OnboardingState }
> {
  const user = await requireUser()
  if (await viewingAsCoach()) return { ok: false, state: { message: READ_ONLY } }
  if (user.role !== 'coach') return { ok: false, state: { message: 'Only coaches can do this.' } }
  return { ok: true, user }
}
...
export async function saveAboutStep(_prev: OnboardingState, formData: FormData): Promise<OnboardingState> {
  const gate = await requireEditingCoach()
  ...
  redirect('/coach/onboarding?step=photo')
}
...
export async function saveSessionsStep(_prev: OnboardingState, formData: FormData): Promise<OnboardingState> {
  ...
  redirect('/coach/onboarding?step=calendar')
}

/** Step 6 — Handbook. Captures the typed signature once, like saveCoachProfile does. */
