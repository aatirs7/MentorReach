 ...
 *
 * A mentor declares recurring weekly hours in their own timezone, plus one-off blackout
 ...
 *
 * FK convention (as everywhere): mentor_id references users.id, never mentor_profiles.id.
 */

/** A recurring weekly availability window, in the mentor's timezone (mentor_profiles.timezone). */
export const mentorAvailabilityRules = pgTable(
  ...
  (t) => [
    ...
    index('mentor_availability_rules_mentor_idx').on(t.mentorId),
  ],
)

/** A full-day exception — the mentor is unavailable this whole date (their local date). */
export const mentorAvailabilityBlackouts = pgTable(
  'mentor_availability_blackouts',
  {
    ...
    /** Calendar date in the mentor's timezone (no time component). */
    day: date('day', { mode: 'string' }).notNull(),

    createdAt: timestamp('created_at', { withTimezone: true, mode: 'date' }).notNull().defaultNow(),
  },
  (t) => [unique('mentor_availability_blackouts_mentor_day_unq').on(t.mentorId, t.day)],
)
 ...
 *
 * UNIQUE(mentor_id, slot_start) makes two concurrent holds on the same start collide, so
...
export const sessionHolds = pgTable(
  'session_holds',
  {
    ...
    offeringId: uuid('offering_id')
      .notNull()
      .references(() => mentorOfferings.id, { onDelete: 'cascade' }),

    linkId: uuid('link_id')
      .notNull()
      .references(() => mentorStudentLinks.id, { onDelete: 'cascade' }),
  ...
  (t) => [
    unique('session_holds_mentor_slot_unq').on(t.mentorId, t.slotStart),
